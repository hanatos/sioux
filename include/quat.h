...
static inline void quat_init(quat_t *q, float w, float x, float y, float z)
{
  q->w = w;
  q->x[0] = x;
  q->x[1] = y;
...
static inline void quat_init_angle(
    quat_t *q,
    const float angle,
    float x,
    float y,
...
static inline void quat_add(quat_t *q, const quat_t *a)
{
  for(int k=0;k<3;k++) q->x[k] += a->x[k];
  q->w += a->w;
}

static inline void quat_muls(quat_t *q, const float s)
{
...
static inline void quat_mul(const quat_t *a, const quat_t *b, quat_t *res)
{
...
static inline void quat_transform_inv(const quat_t *const q, float* p)
{
  // q' * v * q
  quat_t v;
...
static inline void quat_transform(const quat_t *const q, float* p)
{
  // q * v * q'
  quat_t v;
  quat_init(&v, 0.0f, p[0], p[1], p[2]);
...
static inline void quat_normalise(quat_t *q)
{
...
static inline void quat_lerp(const quat_t *const q, const quat_t *const p, const float t, quat_t *res)
{
...
static inline void quat_to_euler(
    const quat_t *q,
    float *roll,
    float *pitch,
    float *yaw)
...
static inline void quat_from_euler(
    quat_t *q,
    float roll,
    float pitch,
    float yaw)
